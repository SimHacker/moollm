...
7. **Skill lookup = local tree walk, then mounted workspace roots.** `inherits: [biome]` is resolved in two phases. **Phase 1** walks the referring skill's directory outward; at each level, two independent searches happen — (a) is there a `skills/<name>/` subdir at this level? (b) does this dir itself have `SKILL.md` with a matching basename? Any directory can host a scoped `skills/`, whether it's a skill or not. **Phase 2** consults all mounted workspace roots (Cursor's mounted repos): `moollm/skills/`, `central/skills/`, customer-specific, distributed, public, private — **equal-citizen skill roots**. First match wins; closer definitions shadow farther ones. Parent-dir containment is a **relatedness signal**; inheritance flows through filesystem containment the way CSS cascades or lexical scope walks enclosing environments. Postel: emit unambiguous fragments (`moollm/skills/biome/`), accept bare names.
