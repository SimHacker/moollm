#!/usr/bin/env python3
"""
Cursor chat/workspace inspector (macOS paths, read-only).
...
REFERENCE SHORTCUTS:
  ...
  Examples:
    cursor-chat list-workspaces            # Shows w1, w2, w3...
    cursor-chat list-composers moollm      # Shows c1, c2, c3... in moollm
    cursor-chat tree                       # Browse workspaces
    cursor-chat tree w3                    # Browse composers in w3
    cursor-chat tree w3.c2                 # Composer details
    cursor-chat tree w3.c2.tools           # Tool calls in that chat
    cursor-chat transcript @1              # Largest composer
    cursor-chat show-workspace moollm
    cursor-chat sql --db moollm --keys mcp
    cursor-chat find cursor-chat
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        prog="cursor-chat",
        description="Inspect Cursor chats/workspaces (read-only).",
...
class CursorChatError(Exception):
    """Base exception for cursor-chat errors."""
    pass

class NotFoundError(CursorChatError):
    """Entity not found (workspace, composer, etc.)."""
    pass

class DatabaseError(CursorChatError):
    """Database access or query error."""
    pass

class ValidationError(CursorChatError):
...
__all__ = [
    # Exceptions
    "CursorChatError", "NotFoundError", "DatabaseError", "ValidationError",
    # Entry points
    "main", "cli",
    # Path constants
...
log = logging.getLogger("cursor-chat")
...
def resolve_composer(ref: str, workspace_ref: Optional[str] = None) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    Accepts:
      ...
      - Name fragment: "cursor-chat", "moollm"
    """
    if not ref:
        return None
...
def cmd_blobs(args):
    ...
    if args.yaml or args.json:
        ...
        print(f"\nUse: cursor-chat blobs --show <hash> to view content")
