...
**Supersession (multi-axis):** **[`schemas/supersession-suggestions.yml`](schemas/supersession-suggestions.yml)** lists **legacy clusters** with four optional axes: **`toward_now`** (north star), **`upwardish`** (credible near-current tradeoffs), **`sideways`** (peer alternatives), **`roots_downward`** (historical foundations—vocabulary, not migration targets). Complements **`gateways.yml`** (transform edges between mechanism ids), not a replacement.
