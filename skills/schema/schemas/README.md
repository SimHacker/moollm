...
| [**gateways.yml**](./gateways.yml) | **Bridges (graph edges)** — JSON↔Zod, XSD↔RNG, JSON↔SQLite, Drescher↔rows, `json-schema`↔`yaml-jazz`, `shell-orchestration`↔`cursor-mirror`, `cursor-mirror`↔`sqlite`, `sqlite`↔`datasette`, `cursor-mirror`↔`datasette`, etc. With `registry.yml`, forms the schemapedia **topology** for transforms and validation layers. |
...
| [**blend-space.yml**](./blend-space.yml) | **Simplicial complex** over mechanisms (joint workflows = higher simplices); **barycentric coordinates** as blend weights; **relative proximity to self** on generality / simplicity / efficiency / influence (anchor-relative); **orthogonal** to supersession meta-axes. |
