...
| [`WORKFLOWS.md`](WORKFLOWS.md) | **A pie menu item is a container for ordered lists of named commands, one list per event** — Kando 3.0's model (2026-09-23), taken whole: hover, select, open and center workflows per item, each with its own quick-select key, and closing the menu as a step whose position sets timing. The webtop already has what Kando lacks, commands with names sent to views that answer, so `delay 500` becomes `await: page.loaded` and "focus if open, else open" is one argument. Workflows are recorded by doing them (the automation frame sees intents, not keystrokes), travel in the view record, and are edited by a frame. Kando's new `send-websocket-message` is the bridge today: a desktop slice sends a named command to a webtop page, starting with the cabinet |
