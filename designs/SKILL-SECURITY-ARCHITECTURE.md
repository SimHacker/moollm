...
Why Self? Because Self's core insight is simplicity. Self is like microcode for object-oriented programming. It's simpler than any of its descendants, yet powerful enough to implement all of them. The LLM has seen Self, JavaScript, Java, Lua, HyperCard, ScriptX/CLOS generics, Lisp Machine Flavors, The Sims advertisements, and COM/IUnknown OLE/IDispatch (C++ vtables in sheep's clothing) extensively in training data. It knows these models. Self is the substrate that unifies them:
