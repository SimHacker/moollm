...
| [DUBLIN-CORE-AND-THE-ADVENTURE-COMPILER.md](DUBLIN-CORE-AND-THE-ADVENTURE-COMPILER.md) | The strict machine-executable core under the rich semantic overlay — natural-language guards compiled to `guard_js` by an LLM resolving linter warnings; instance-first, not yet lifted; the Scott Adams lineage |
| [HOMOICONICITY.md](HOMOICONICITY.md) | Does MOOLLM have it? Three of the four benefits, and the fourth is where PostScript was strongest. The forgotten half — PostScript's *environment* was also data (the dictionary stack), which is why the definition editor could exist, and which even Lisp closures fail. The hole located precisely: weights and assembled context are not in the repo. Why no single official language is needed (the naming-and-interface grammar plays that role, as a structural type system over the filesystem), why *multi-interpreter isomorphism* is the right target instead, and the constructive path — make the environment an artifact |
| [ENDOSYMBIOSIS.md](ENDOSYMBIOSIS.md) | Composition the Margulis way — engulf working systems whole, let them keep their own genome (source, in its original language); gene transfer to the nucleus as crystallization; the specimens: Cross-Platform Troll, Wumpus's game cartridge, the coatroom grue, cross-game pieces, ebike safari, real↔imaginary map doors |
...
[readings/2026-07-14-object-system-readme-reading.md](readings/2026-07-14-object-system-readme-reading.md),
with a timestamped overlay track
([.yml](readings/2026-07-14-object-system-readme-reading.yml)) modeling media-event
injections: links, definitions, and preconfigured simulations synced to the video.
