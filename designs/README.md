...
| [MORPHIC-LINEAGE.md](./MORPHIC-LINEAGE.md) | Every system that ran on Morphic or rebuilt it: Self → Squeak → Tweak/Croquet, Cuis Morphic 3, Lively Kernel→Web→4→next, morphic.js→Snap!, GP→MicroBlocks, MorphiCL/Blocky. What travels (morph tree, World/Hand, halos, no editor mode) and what dies every time (multiple inheritance, abstraction for reuse) |
...
| [PROSTHETICS.md](./PROSTHETICS.md) | **What this stack is, stated once.** Five prosthetics, each aimed at a different unreliable faculty: moollm for a *society of mind*, WillWrightShowForFood for *memory of other people*, webtop for *attention*, MicropolisCore for *model-skepticism*, GitHub-as-MMORPG for *community*. Science had to be invented, so confidence in the unaided apparatus is the symptom rather than the license — the inversion that recurs in every row. Three of the five are Minsky's own mechanisms externalized (agents, K-lines, level-bands) and two are conditions rather than faculties, which is stated instead of forced. The distinguishing question is **whose loop is this closing**, answerable here because the motives are in YAML with an author and a date; a prosthetic needs no telos of its own, which is why the missing closure is the spec and not a gap. Carries the anti-prosthetic table — the commercial opposite of each row, better funded, closing its loop on engagement — and four tests, plus *we'll do it live* as the repo-show register |
...
```mermaid
graph LR
    D[designs/] -->|crystallizes into| K[kernel/]
    D -->|crystallizes into| S[skills/]
    D -->|shapes into| SC[schemas/]
    
    PLAY[🎮 PLAY] -->|happens in| D
    LEARN[📚 LEARN] -->|extracts to| K
    LIFT[🚀 LIFT] -->|refines into| S
```
