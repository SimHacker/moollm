...
Here is the concrete tour: the [Lane Neverending Photo Archive](https://github.com/SimHacker/moollm/blob/don-adventure-4-run-1/examples/adventure-4/street/lane-neverending/slideshow/SLIDESHOW.md) is a five-location slideshow built from semantic stereo image prompt generation (`PHOTO.yml` + `PHOTO.md`). Two eyes, one image: the YAML provides structure (composition, lighting, objects, colors) while the Markdown provides poetry (mood, narrative, sensory detail). The LLM synthesizes both into a coherent prompt that captures what words alone cannot. After generation, mining layers analyze the result across multiple lenses (technical, narrative, cultural, phenomenology, absurdist) and feed back into the next iteration.
