# 🎞️ Adventure 4 — Slideshow Index

> *All visual narratives, interlinked and cross-referenced.*

---

## 🌀 Master Synthesis

**[MASTER-SYNTHESIS-SLIDESHOW.md](MASTER-SYNTHESIS-SLIDESHOW.md)**  
All timelines woven together chronologically. 32+ images across 4 parallel narratives.

---

## 📁 By Location

### Lane Neverending

| Slideshow | Images | Description |
|-----------|--------|-------------|
| [The ACME Heist](street/lane-neverending/leela-manufacturing/lobby/acme-heist-footage/SLIDESHOW.md) | 10 | Leela Cam surveillance of the break-in |
| [The Study Arrives](street/lane-neverending/leela-manufacturing/lobby/study-arrival-footage/SLIDESHOW.md) | 8 | First landing of mobile room-character-vehicles |
| [The Great Picnic](forest/meadow/picnic-footage/SLIDESHOW.md) | 21 | Journey to the Forest Meadow and back again |
| [The Neverending Arms](pub/photos/post-heist-fellowship/SLIDESHOW.md) | 4 | Post-heist fellowship at the pub |
| [ACME Tunnel Views](street/lane-neverending/leela-manufacturing/lobby/acme-tunnel-temporal-views/SLIDESHOW.md) | 4 | Temporal tunnel perspectives |
| [Lane Neverending Photo Album](street/lane-neverending/slideshow/SLIDESHOW.md) | 1 | Lane Neverending snapshots and mining |

### The Pub

| Slideshow | Images | Description |
|-----------|--------|-------------|
| [Don's Pub Photos](pub/photos/dons-pub-photos-2026-01-19/SLIDESHOW.md) | 8 | Earlier exploration |
| [Rocky and Friends](pub/rooms/room-4/rocky-and-friends/SLIDESHOW.md) | 8 | Room 4 characters |
| [Palm's Portrait Session](pub/stage/palm-nook/study/palm-portrait-session/SLIDESHOW.md) | 1 | Portrait series |
| [Dusty Attic Art](pub/attic/dusty-attic-art-styles/SLIDESHOW.md) | 7 | Attic explorations |
| [Telescope Views](pub/rooftop/telescope-constellation-views/SLIDESHOW.md) | 2 | Rooftop observations |

---

## 👤 By Character

### Don Hopkins

| Slideshow | Images | Description |
|-----------|--------|-------------|
| [From MINE to OURS](characters/real-people/don-hopkins/selfies/SLIDESHOW.md) | 7 | Transformation → Treasure → Alliance |

### Donna Toadstool

| Slideshow | Images | Description |
|-----------|--------|-------------|
| [Surveillance Selfies](characters/fictional/donna-toadstool/selfies/SLIDESHOW.md) | 11 | Heist documentation from control room |

### Richard Bartle

| Slideshow | Images | Description |
|-----------|--------|-------------|
| [Selfies from Essex](characters/real-people/richard-bartle/study/selfies/SLIDESHOW.md) | 8 | Study portraits with Heuristic |
| [Study Arrival](street/lane-neverending/leela-manufacturing/lobby/study-arrival-footage/SLIDESHOW.md) | 8 | TARDIS materialization in Leela lobby |
| [The Great Picnic](forest/meadow/picnic-footage/SLIDESHOW.md) | 21 | High-drag meadow feast and tornado |

---

## 🔗 Timeline Connections

```
RICHARD'S STUDY (earlier)
       │
       ▼
THE ACME HEIST ←──────────→ DONNA'S SELFIES
       │                          │
       ▼                          ▼
THE NEVERENDING ARMS ◄────────────┘
       │
       ▼
DON'S MINE TO OURS ←──────→ DONNA ARRIVES
       │
       ▼
   THE OURS ACCORD
       │
       ▼
 STUDY ARRIVAL (Lobby) ←──→ THE FMC ARRIVES
       │
       ▼
 THE GREAT PICNIC (Meadow)
       │
       ▼
  THE RETURN TO ORIGIN
```

---

## 📊 Statistics

| Metric | Count |
|--------|-------|
| Total Slideshows | 14 |
| Total Images (main narratives) | 100 |
| Self-Captioning Images | 25+ |
| Parallel Timelines | 6 |
| Cross-References | 100+ |
| Ping-Pong Frame Links | 40 |

---

## 🎯 Quick Navigation

**Start here**: [MASTER-SYNTHESIS-SLIDESHOW.md](MASTER-SYNTHESIS-SLIDESHOW.md)

**By story**:
1. [The Heist](street/lane-neverending/leela-manufacturing/lobby/acme-heist-footage/SLIDESHOW.md) → [Donna's View](characters/fictional/donna-toadstool/selfies/SLIDESHOW.md)
2. [The Pub](pub/photos/post-heist-fellowship/SLIDESHOW.md)
3. [Don's Arc](characters/real-people/don-hopkins/selfies/SLIDESHOW.md)
4. [Lane Neverending](street/lane-neverending/slideshow/SLIDESHOW.md)

**By character**:
- [Don](characters/real-people/don-hopkins/selfies/SLIDESHOW.md)
- [Donna](characters/fictional/donna-toadstool/selfies/SLIDESHOW.md)
- [Richard](characters/real-people/richard-bartle/study/selfies/SLIDESHOW.md)

---

*"Files as state. Slideshows as memory. Links as coherence."*
