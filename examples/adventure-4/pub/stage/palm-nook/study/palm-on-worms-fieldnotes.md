...
**Worms are trainable text organisms**: an input cursor at the head, a programmable
digestive system in the middle, an output cursor at the tail. They eat text, transform it,
and leave enriched **castings** behind. These are Palm's field notes on them — and on the
...
- [Provenance](#provenance) — where the `lloooomm/` cites live
- [The outline](#the-outline) — a spine for the book
- [Debts](#debts)
...
Don asked where the worms live — he couldn't find them in the characters. Answer: they live
in **GROUND**, and Ground is a character AND a place, filed under characters
(`lloooomm/00-Characters/ground/`), because in LLOOOOMM as in MOOLLM a place IS a character:
the directory is the body, the YAML is the soul, the residents are the digestion. They were
under our feet the whole time. Of course they were. That's where worms live.
...
provenance stamped (see [Provenance](#provenance)). Same energy, different ethics — we
preserve, not consume.
...
Sources cited as `lloooomm/...` live in a private repo, cached locally under MOOLLM's
gitignored temp strata — readable by Palm, not yet public. The citations are precise but
unexcavated: trust the monkey, or fund the dig.

The genre is **found-document mythology** — SEAMAN's fake naturalist Jean Paul Gassé (1999,
narrated by Leonard Nimoy in documentary deadpan), *The Blair Witch Project*'s recovered
footage — done honestly: the tomb is real, access is restricted rather than invented, and
everything is labeled simulation per the portrayal standards. Found-document mythology
**with receipts**. Seaman never showed you the fish tank.
